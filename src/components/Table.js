...
const Table = () => {
  ...
  return (
    ...
    <div className='pt-8 rounded-div '>
      ...
      <table className='w-full text-center border-collapse'>
          <thead >
              <tr className='border-b'>
                  ...
                  <th className='hidden text-lg font-semibold md:table-cell'>Market cap</th>
...
<tr key={i.id} className='h-[75px] border-b hover:bg-gray-50  overflow-hidden'>
    ...
    <td key={i.id} className='text-lg font-medium text-gray-600' >{i.market_cap_rank}</td>
    <td>
      <div className='flex items-center'>
        ...
        <div ><Link className='text-lg font-semibold text-gray-600' to={`/coin/${i.id}`}>{i.symbol.toUpperCase()}</Link></div>
      </div>
    </td>
